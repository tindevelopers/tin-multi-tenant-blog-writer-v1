...
/**
 * Test 100 Word Blog Generation
 * Tests with main topics and subtopics using the working endpoint
 */

const https = require('https');

const BACKEND_URL = 'https://blog-writer-api-dev-613248238610.europe-west9.run.app';
...
async function test100WordBlog() {
  console.log('========================================');
  console.log('100 Word Blog Generation Test');
  console.log('========================================');
  console.log(`Backend URL: ${BACKEND_URL}`);
  console.log(`Endpoint: ${ENDPOINT}`);
  console.log(`Target Word Count: 100 words`);
  console.log('========================================\n');
  ...
  try {
    ...
    console.log(`  Target Word Count: 100 words`);
    console.log(`  Difference: ${wordCount - 100} words`);
    console.log(`  Within Tolerance (±25%): ${wordCount >= 75 && wordCount <= 125 ? '✅ Yes' : '❌ No'}`);
    ...
    if (response.body.seo_metadata) {
      ...
      if (response.body.seo_metadata.subtopics && response.body.seo_metadata.subtopics.length > 0) {
        console.log(`  Subtopics Generated: ${response.body.seo_metadata.subtopics.length}`);
        response.body.seo_metadata.subtopics.forEach((topic, idx) => {
          console.log(`    ${idx + 1}. ${topic}`);
        });
      }
      if (response.body.seo_metadata.semantic_keywords && response.body.seo_metadata.semantic_keywords.length > 0) {
        console.log(`  Semantic Keywords: ${response.body.seo_metadata.semantic_keywords.length}`);
        console.log(`    ${response.body.seo_metadata.semantic_keywords.slice(0, 5).join(', ')}...`);
      }
    ...
    const checks = {
      ...
      wordCountValid: wordCount >= 75 && wordCount <= 125,
    ...
    console.log(`  Word count in range (75-125): ${checks.wordCountValid ? '✅' : '❌'}`);
    ...
    if (allPassed) {
      console.log('\n🎉 SUCCESS: 100-word blog generated successfully!');
      console.log(`\n📝 Full Content:`);
      console.log(response.body.content);
      console.log(`\n📝 Meta Description:`);
      console.log(response.body.meta_description || 'N/A');
...
test100WordBlog().catch(console.error);
